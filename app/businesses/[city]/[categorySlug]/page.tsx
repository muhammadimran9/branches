...
export default async function CityCategoryPage(props: { params: Promise<{ city: string; categorySlug: string }> }) {
  ...
  try {
    const q = query(
      ...
      where('city', '==', cityName),
      where('category', '==', params.categorySlug),
      limit(40)
    )
  ...
  const pageUrl = `${BASE_URL}/businesses/${params.city}/${params.categorySlug}`
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 2, name: category.name, item: `${BASE_URL}/category/${params.categorySlug}` },
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
          ...
          <section className="mb-8">
            <div className="flex flex-wrap gap-3">
              ...
              <Link href={`/category/${params.categorySlug}`} className="inline-flex items-center gap-1.5 px-4 py-2 bg-white rounded-lg border border-gray-200 text-sm font-medium text-gray-700 hover:border-[#60a5fa] hover:text-[#60a5fa] transition-colors shadow-sm">
          ...
          <section className="mb-12">
            ...
            <div className="flex flex-wrap gap-3">
              {CITIES.filter(c => c !== cityName).slice(0, 15).map(city => (
                <Link
                  key={city}
                  href={`/businesses/${city.toLowerCase().replace(/ /g, '-')}/${params.categorySlug}`}
